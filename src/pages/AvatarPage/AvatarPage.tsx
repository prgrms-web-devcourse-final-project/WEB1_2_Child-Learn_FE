...
function AvatarPage() {
  ...
  useEffect(() => {
    setMarketItems((prevMarketItems) => {
      if (prevMarketItems.length > 0) return prevMarketItems;
      ...
      // `purchased` 필드를 현재 상태에 따라 설정
      return initialItems.map((item) => ({
        ...item,
        purchased: prevMarketItems.find((prevItem) => prevItem.prd_id === item.prd_id)?.purchased || false,
      }));
    });
